...
count=np.arange(500,2000,200)
count1=np.arange(1,200,20)
for k in count:
    for k1 in count1:
        names.append("Random_Forest: estimators " + str(k) + "  depth"+ str(k1))
        classifiers.append(RandomForestClassifier(max_depth=k1, n_estimators=k, max_features=1))
...
my_df.to_csv('Random_forest.csv', index=False, header=False)