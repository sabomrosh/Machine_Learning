...
for k in count:
    for k1 in count1:
        names.append("MLP_Tanh: Hidden " + str(k) + "  alpha"+ str(k1))
        classifiers.append(MLPClassifier(activation='tanh',alpha=k1,hidden_layer_sizes=(k,)))
    names.append("MLP_Tanh: Hidden " + str(k) + "  alpha"+ str(1))
    classifiers.append(MLPClassifier(activation='tanh',alpha=1,hidden_layer_sizes=(k,)))
...
my_df.to_csv('MLP_Tanh.csv', index=False, header=False)