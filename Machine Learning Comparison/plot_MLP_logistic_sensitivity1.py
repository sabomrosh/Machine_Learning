...
count=np.arange(1,500,10)
count1=np.arange(0,0.2,0.01)
for k in count:
    for k1 in count1:
        names.append("MLP_Logistic: Hidden " + str(k) + "  alpha"+ str(k1))
        classifiers.append(MLPClassifier(activation='logistic',alpha=k1,hidden_layer_sizes=(k,)))
    names.append("MLP_Logistic: Hidden " + str(k) + "  alpha"+ str(1))
    classifiers.append(MLPClassifier(activation='logistic',alpha=1,hidden_layer_sizes=(k,)))
...
my_df.to_csv('MLP_logistic.csv', index=False, header=False)