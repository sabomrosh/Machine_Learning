...
for k in count:
    for k1 in count1:
        names.append("MLP_RELU: Hidden " + str(k) + "  alpha"+ str(k1))
        classifiers.append(MLPClassifier(alpha=k1,hidden_layer_sizes=(k,)))
    names.append("MLP_RELU: Hidden " + str(k) + "  alpha"+ str(1))
    classifiers.append(MLPClassifier(alpha=1,hidden_layer_sizes=(k,)))
...
my_df.to_csv('MLP_relu.csv', index=False, header=False)