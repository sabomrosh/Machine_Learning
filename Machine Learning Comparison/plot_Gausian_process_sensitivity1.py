...
count=np.arange(3,7,1)
count1=np.arange(0.5,2,0.5)
for k in count:
    for k1 in count1:
        names.append("Gaussian_Process: Gain " + str(k) + "  RBF_exp"+ str(k1))
        classifiers.append(GaussianProcessClassifier(k * RBF(k1), max_iter_predict=100,warm_start=True))
...
my_df.to_csv('Gaussian_processG3-6.csv', index=False, header=False)